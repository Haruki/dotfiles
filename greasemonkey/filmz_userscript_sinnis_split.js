(async function() {
  ...
  function getScore() {
    ...
    var ratingSpanSearchResult = document.evaluate(
  ...
  function getReleaseYear() {
    var year;
    var releaseAnchorSearchResult = document.evaluate(
      "//span[@id='titleYear']//a",
      document,
      null,
      XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
    ...
    if (releaseAnchorSearchResult.snapshotLength > 0) {
      ...
      releaseAnchorSearchResult = document.evaluate(
        "//span[@class='nobr']",
        document,
        null,
        XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
  ...
  function getReleaseDate(releaseYear, releaseMonth, releaseDay) {
    var date;
    var dateAnchorSearchResult = document.evaluate(
      "//div[@class='title_wrapper']//meta[@itemprop='datePublished']",
      document,
      null,
      XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
      null
    ...
    date = new Date(
      releaseMonth + " " + releaseDay + ", " + releaseYear + " 00:00:00"
    );
    console.log(date);
    console.log(
  ...
  function getNameDeutsch() {
    ...
    var name;
    var deutschH1SearchResult = document.evaluate(
      "//div[@class='title_wrapper']//h1[@itemprop='name']",
      document,
      null,
      XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
      null
  ...
  function getNameOriginal(nameDeutsch) {
    var name;
    var originalSearchResult = document.evaluate(
      "//div[@class='title_wrapper']//div[@class='originalTitle']",
      document,
      null,
      XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,
      null
  ...
  function createAddForm() {
    ...
    createElementWithAttributes(
      form,
      "input",
      { type: "button", value: "ADD MOVIE", id: "addButton" },
      true
    );
    document
      .getElementById("addButton")
      .addEventListener("click", addMovie, false);
    // socialDivElement.innerHTML =
  ...
  function processTheMovieDbData(result) {
    ...
    if (response.code) {
      ...
      //Datenvariablen fÃ¼r movie daten fÃ¼llen:
  ...
  function populateUpdateForm(movie) {
    ...
    addTextLine(
      parent,
      "Added: " + formatDate(new Date(movie.createTime)),
      true
    );
    addTextLine(
      parent,
      "SeenTime: " + formatDate(new Date(movie.seenTime)),
      true
    );
    addTextLine(
      parent,
      "Released: " + formatDate(new Date(movie.releaseDate)),
      true
    );
