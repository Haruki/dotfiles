...
//try fetch filmz api
let response = await fetch(host + "/filmz/find?imdbCode=" + pageMovie.imdbcode, {method:'GET',
        //headers: headers,
...
if (typeof GM_getValue('filmz.user') == 'undefined' || typeof GM_getValue('filmz.pass') == 'undefined') {
...
fetch(host + "/filmz/find?imdbCode=" + pageMovie.imdbcode, {method:'GET',
        headers: headers,
       })
...
function getTheMovieDbData(imdbcode) {
	GM_xmlhttpRequest({
	  method:"GET",
	  url:themoviedbhost + "/3/find/tt" + imdbcode + "?api_key=caf9f8363a15942e96e2678c36b80373&language=en-US&external_source=imdb_id",
	  headers:{
...
function processTheMovieDbData(result) {
	...
	if (response.code) {
        ...
        if ((typeof pageMovie.namedeutsch == 'undefined') || pageMovie.namedeutsch.length == 0) {
